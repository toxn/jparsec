...
public class RecoverTest {
    ...
    static final Parser<Token> PHRASE = TERMS.token("foo")
	...
	static final Parser<String> SYNTAX2 = PHRASE.source()
			.recover(ped -> String.format("%s at %d, encountered %s, unexpected %s, expected %s",
					ped.getFailureMessage(), ped.getIndex(), ped.getEncountered(), ped.getUnexpected(),
					ped.getExpected()), Parsers.never())
